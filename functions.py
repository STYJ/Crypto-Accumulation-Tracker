...
# Function to process CMC's api to retrieve the name of coin based on symbol
# provided
def updateDB(database):
    ...
    # Get tbody
    data = list(
        list(soup.find('table', id='currencies-all').children)[3].children)
    ...
    while(i < length):
        entry = list(list(data[i].children)[3].children)
        ticker = entry[3].get_text()
        try:
            database[ticker]
        except KeyError:
            suffix = entry[5]['href']
            database[ticker] = suffix
        finally:
            i = i + 2
...
def getSource(type, *args):
    ...
    if type == 'exchange':
        ...
        url = 'https://coinmarketcap.com' + database[args[0]]
    elif type == 'history':
    ...
    if soup:
        ...
        while(i < len(rows)):
            ...
            vol = int(
                re.sub("[^\d\.]",
                       "",
                       (list(list(rows[i])[7].children)[1]).get_text()))
        ...
        # If you're using the cache version, uncomment the line below and
        # comment the one after, vice versa.
...
def concatExchanges(exchanges):
    ...
    # Based on what I tested, bitcoin (which should be listed on the most
    # exchanges) do not even have that many exchanges that will require a
    # second string of 4096 characters so the entire code below is not
    # necessary for now.
    # i = 0
    # j = 0
    ...
    # 		lenString = lenString + len(list_of_exchanges[j]) + 2
    # 		j += 1
    # 	else:
...
def exchangeWrapper(bot, update, args):
    ...
    if len(args) != 1:
        ...
        try:
            ...
            exchanges = getExchangeNoCache(coin)
...
def updateCacheWrapper(bot, update):
    ...
    bot.send_message(chat_id=update.message.chat_id,
                     text="Exchanges and vol for all coins have been updated!",
                     reply_to_message_id=update.message.message_id)
...
def analyseWrapper(bot, update, args):
    ...
    if len(args) != 2:
        ...
        if (coin in database) and day > 0:
            ...
            bot.send_message(chat_id=update.message.chat_id,
                             text=args[0] + " cannot be found in DB, please" +
                             " run the updateDB command or check that you've" +
